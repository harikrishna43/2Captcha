...
namespace _2Captcha
{
    public class TwoCaptcha
    {
            ...
            try
            {
                ...
                else
                {
                    if (response.Substring(0, 3) == "OK|")
                    {
                        ...
                        for (int i = 0; i < 24; i++)
                        {
                            HttpWebRequest getAnswer = (HttpWebRequest)WebRequest.Create("http://2captcha.com/res.php?key=" + APIKey + "&action=get&id=" + captchaID);
        ...
        public bool GetBalance(out string result)
        {
            try
            {
                HttpWebRequest getAnswer = (HttpWebRequest)WebRequest.Create("http://2captcha.com/res.php?key=" + APIKey + "&action=getbalance");
                using (HttpWebResponse answerResp = getAnswer.GetResponse() as HttpWebResponse)
                using (StreamReader answerStream = new StreamReader(answerResp.GetResponseStream()))
                {
                    string answerResponse = answerStream.ReadToEnd();
            ...
            {
                result = "Request not sent. Try again";
            }
